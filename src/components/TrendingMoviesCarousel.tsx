...
export function TrendingMoviesCarousel({ movies, onMovieSelect, isGrid = false, onToggleGrid, userLibrary, onLoadMore }: TrendingMoviesCarouselProps) {
      ...
      <motion.div layout className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <button
            ...
            className="text-tertiary-fixed-dim hover:text-tertiary font-body-md text-body-md flex items-center gap-1 transition-colors bg-transparent border-none cursor-pointer"
          >
      ...
      >
        <AnimatePresence mode="popLayout">
          {movies.map((movie, index) => {
            ...
            const inWatchlist = userLibrary?.watchlists?.some(w => w.movieId === movie.id);
            ...
            return (
              ...
              >
                <motion.div whileHover={{ scale: 1.05 }} className="w-full h-full transition-transform duration-300">
                  {movie.poster_path ? (
                    <img
                      alt={title}
                  ...
                  <div className="absolute top-2 left-2 flex flex-col gap-1 items-start z-20 pointer-events-none">
                    ...
                    {index < 10 && !isGrid && (
                      <div className="bg-primary-container text-on-primary font-label-sm text-[10px] px-2 py-1 rounded-full font-bold">
                        TOP {index + 1}
                      </div>
                    )}
                  </div>
